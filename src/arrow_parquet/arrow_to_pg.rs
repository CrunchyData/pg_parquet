...
use crate::{
    ...
    type_compat::{
        ...
        map::{is_crunchy_map_type, CrunchyMap},
...
pub(crate) trait ArrowArrayToPgType<A: From<ArrayData>, T: IntoDatum> {
    fn to_pg_type(array: A, context: &ArrowToPgAttributeContext) -> Option<T>;
}
...
impl ArrowToPgAttributeContext {
    pub(crate) fn new(name: &str, typoid: Oid, typmod: i32, fields: Fields) -> Self {
        ...
        if is_array {
            ...
            is_crunchy_map = is_crunchy_map_type(element_typoid);
            ...
            is_crunchy_map = is_crunchy_map_type(typoid);
...
macro_rules! to_pg_datum {
    ($arrow_array_type:ty, $pg_type:ty, $arrow_array:expr, $attribute_context:expr) => {{
        let val = <$pg_type as ArrowArrayToPgType<$arrow_array_type, $pg_type>>::to_pg_type(
            $arrow_array.into(),
            $attribute_context,
        );
    ...
    match attribute_context.typoid {
        ...
        _ => {
            if attribute_context.is_composite {
                ...
                to_pg_datum!(MapArray, CrunchyMap, primitive_array, attribute_context)
