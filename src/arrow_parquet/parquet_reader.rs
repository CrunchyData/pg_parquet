...
use super::{
    ...
    schema_visitor::ensure_arrow_schema_match_tupledesc,
...
impl ParquetReaderContext {
    pub(crate) fn new(uri: Url, tupledesc: &PgTupleDesc) -> Self {
        // PostGIS and Crunchy Map contexts are used throughout reading the parquet file.
    ...
    // bytes_in_buffer returns the number of the bytes that are not yet consumed
