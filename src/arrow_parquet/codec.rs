...
impl TryFrom<Url> for ParquetCodecOption {
    type Error = String;
    ...
    fn try_from(value: Url) -> Result<Self, Self::Error> {
        ...
        if path.ends_with(".parquet") {
            Ok(ParquetCodecOption::Uncompressed)
        } else if path.ends_with(".parquet.snappy") {
            Ok(ParquetCodecOption::Snappy)
        } else if path.ends_with(".parquet.gzip") {
            Ok(ParquetCodecOption::Gzip)
        } else if path.ends_with(".parquet.lz4") {
            Ok(ParquetCodecOption::Lz4)
        } else if path.ends_with(".parquet.lz4_raw") {
            Ok(ParquetCodecOption::Lz4raw)
        } else if path.ends_with(".parquet.br") {
            Ok(ParquetCodecOption::Brotli)
        } else if path.ends_with(".parquet.zstd") {
            Ok(ParquetCodecOption::Zstd)
        } else {
